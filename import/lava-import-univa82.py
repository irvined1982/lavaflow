...
parser = argparse.ArgumentParser(description='Import Univa Grid Engine 8.2 Accounting Files into LavaFlow')
...
url = args.url.rstrip("/")
url += "/clusters/%s/import/univa82" % cluster_name
...
acct_file = UGEAccountFile(fh)
row_num = 0
row_data = []
for row in acct_file:
    row_num += 1
    if len(row_data) > 0 and len(row_data) % args.chunk_size == 0:
        logging.debug("Importing %s rows" % row_num)
        upload(row_data)
        row_data = []
    row_data.append(row.to_dict())
fh.close()
