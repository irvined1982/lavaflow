...
def homeView(request):
	return render_to_response("lavaFlow/homeView.html",{},context_instance=RequestContext(request))
...
def clusterOverviewModule(request, startTime,endTime):
	clusters=Run.objects.filter(element__job__submitTime__lte=endTime, endTime__gte=startTime).values('element__job__cluster__id').annotate(
	...
	return render_to_response("lavaFlow/modules/clusterOverview.html",{'clusters':clusters},context_instance=RequestContext(request))

def busyUsersModule(request, startTime, endTime, field):
	...
	users=Run.objects.filter(element__job__submitTime__lte=endTime, endTime__gte=startTime).values('element__job__user__id').annotate(
...
def bestHostModule(request, startTime, endTime):
	hosts=Run.objects.filter(element__job__submitTime__lte=endTime, endTime__gte=startTime, runFinishInfo__jStatus__jStatus="Done").values('executions__host').annotate(
			numRuns=Count('id'),
			sumCpu=Sum('cpuTime'),
...
def worstHostModule(request, startTime, endTime):
	hosts=Run.objects.exclude(element__job__submitTime__lte=endTime, endTime__gte=startTime, runFinishInfo__jStatus__jStatus="Done").exclude(executions__host__isnull=True).values('executions__host').annotate(
			numRuns=Count('id'),
			sumCpu=Sum('cpuTime'),
		).order_by('-numRuns')[0:10]
	for h in hosts:
...
def busySubmitModule(request, startTime, endTime):
	hosts=Run.objects.filter(element__job__submitTime__lte=endTime, endTime__gte=startTime).values('element__job__submitHost').annotate(
			numRuns=Count('id'),
			sumCpu=Sum('cpuTime'),
			).order_by('-numRuns')[0:10]
	for h in hosts:
...
def jobSizeChartModule(request, startTime, endTime):
	rows=Run.objects.filter(element__job__submitTime__lte=endTime, endTime__gte=startTime).values('numProcessors').annotate(
...
def jobSizeTableModule(request, startTime, endTime):
	rows=Run.objects.filter(element__job__submitTime__lte=endTime, endTime__gte=startTime).values('numProcessors').annotate(
			numRuns=Count('numProcessors'),
			sumPendTime=Sum('pendTime'),
			sumWallTime=Sum('wallTime'),
	...
	return render_to_response("lavaFlow/modules/jobSizeTable.html",{'rows':rows},context_instance=RequestContext(request))

def jobExitTableModule(request, startTime, endTime):
	info=Run.objects.filter(
	...
	return render_to_response("lavaFlow/modules/jobExitTable.html",{'rows':info},context_instance=RequestContext(request))

def jobExitChartModule(request, startTime, endTime):
...
def jobListModule(request, startTime, endTime):
...
def utilizationModule(request, startTime, endTime):
	util=Run.objects.filter(
				element__job__submitTime__lte=endTime, 
				endTime__gte=startTime
			)
